...
test("ensure that stats file is modified", async () => {
  const child = child_process.exec(
    "docker run -v $PWD/test-crawls:/crawls webrecorder/crawlertrix crawl --url https://old.webrecorder.net/ --generateWACZ --text --limit 3 --exclude community --collection file-stats --statsFilename progress.json",
  );
