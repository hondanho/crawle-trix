...
export class WARCWriter implements IndexerOffsetLength {
  ...
  private async initFH() {
    ...
    if (!fh) {
      const warcPath = path.join(this.archivesDir, this.filename);
      fs.mkdirSync(path.dirname(warcPath), { recursive: true });
      fh = fs.createWriteStream(path.join(this.archivesDir, this.filename), {
        flags: "a",
      });
    }
    if (!this.cdxFH && this.warcCdxDir) {
      const cdxPath = path.join(this.warcCdxDir, this.filename + ".cdx");
